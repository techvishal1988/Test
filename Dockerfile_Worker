...
WORKDIR /src
COPY ["OrganizationSolution/Source/App/Geography.Worker/Geography.Worker.csproj", "OrganizationSolution/Source/App/Geography.Worker/"]
...
RUN dotnet restore "./OrganizationSolution/Source/App/Geography.Worker/./Geography.Worker.csproj"
COPY . .
WORKDIR "/src/OrganizationSolution/Source/App/Geography.Worker"
RUN dotnet build "./Geography.Worker.csproj" -c $BUILD_CONFIGURATION -o /app/build

FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./Geography.Worker.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "Geography.Worker.dll"]
